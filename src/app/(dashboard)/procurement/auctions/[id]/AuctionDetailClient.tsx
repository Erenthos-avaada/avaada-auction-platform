...
function Countdown({ endTime, onExpire }: { endTime: string; onExpire?: () => void }) {
  ...
  return (
    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "1.1rem", fontWeight: 600, color: urgent ? "var(--danger)" : "var(--accent)" }}>
      {display}
    </span>
  );
}
...
export default function AuctionDetailClient({ auction: initial, initialBids, vendors, invitedIds, role }: {
  ...
  const sendInvites = async () => {
    ...
    setInviteMsg(`✓ ${data.invited} vendor(s) invited successfully`);
    setInviting(false);
  };
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
              <Link href="/procurement/auctions" style={{ fontSize: "0.78rem", color: "var(--text3)", textDecoration: "none" }}>← Auctions</Link>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div className="anim-up d1" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px" }}>
            ...
            <div className="stat-card">
              ...
              <p className="stat-val" style={{ fontSize: "1.4rem" }}>{bids.length}</p>
              <p className="stat-sub">{new Set(bids.map((b: any) => b.vendorId)).size} vendors</p>
            </div>
            <div className="stat-card">
          ...
          <div className="anim-up d2 surface" style={{ overflow: "hidden" }}>
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <p style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)" }}>Live Bid Ladder</p>
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                {auction.status === "ACTIVE" && <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: "var(--success)", display: "inline-block", animation: "pulse 2s infinite" }} />}
                <span style={{ fontSize: "0.72rem", color: "var(--text3)" }}>{auction.status === "ACTIVE" ? "Live" : auction.status}</span>
              </div>
            </div>
            {bids.length === 0
          ...
          <div className="anim-up d1 surface" style={{ padding: "18px 20px" }}>
            ...
            {[
              ...
              <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid var(--border)" }}>
                ...
                <span style={{ fontSize: "0.78rem", color: "var(--text)", fontWeight: 500 }}>{v}</span>
              </div>
            ))}
            {auction.description && (
              <p style={{ fontSize: "0.78rem", color: "var(--text2)", marginTop: "12px", lineHeight: 1.6 }}>{auction.description}</p>
            )}
          </div>
              ...
              {showInvite && (
                <>
                  <div style={{ maxHeight: "200px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "4px", marginBottom: "10px" }}>
                    {vendors.map((v: any) => (
                      <label key={v.id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px 8px", borderRadius: "var(--radius-sm)", cursor: "pointer", background: selected.includes(v.id) ? "var(--accent-bg)" : "transparent", border: selected.includes(v.id) ? "1px solid var(--border2)" : "1px solid transparent", transition: "all 0.15s" }}>
                        <input type="checkbox" checked={selected.includes(v.id)} onChange={() => setSelected(s => s.includes(v.id) ? s.filter(x => x !== v.id) : [...s, v.id])} style={{ accentColor: "var(--accent)" }} />
